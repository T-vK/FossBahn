...
CI caches `fdroid/repo/` and `fdroid/archive/` between releases. Each publish runs [.github/scripts/sync-fdroid-apks-from-releases.sh](../.github/scripts/sync-fdroid-apks-from-releases.sh) to download missing GitHub Release APKs, then [.github/scripts/prune-fdroid-broken-apks.sh](../.github/scripts/prune-fdroid-broken-apks.sh) (known-bad version codes + `apksigner verify` only — never androguard, which false-positives). CI **fails** if the index would list zero packages. `repo_maxage: 0` keeps versions by age; `archive_older: 100` moves only very old builds to `archive/` (still on Pages). Each publish copies **both** `repo/` and `archive/` to GitHub Pages. The repo index signing key is copied each run from [.github/signing/fdroid-repo.p12](../.github/signing/fdroid-repo.p12) (public, like the APK CI key).

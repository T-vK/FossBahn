...
CI caches `fdroid/repo/` and `fdroid/archive/` between releases. Each publish runs [.github/scripts/sync-fdroid-apks-from-releases.sh](../.github/scripts/sync-fdroid-apks-from-releases.sh) to download **every** missing GitHub Release APK, then [.github/scripts/sanitize-fdroid-apks.sh](../.github/scripts/sanitize-fdroid-apks.sh) to quarantine APKs that break `fdroid update` (e.g. legacy signing blocks). `repo_maxage: 0` keeps versions by age; `archive_older: 100` moves only very old builds to `archive/` (still on Pages). Each publish copies **both** `repo/` and `archive/` to GitHub Pages. The repo index signing key is copied each run from [.github/signing/fdroid-repo.p12](../.github/signing/fdroid-repo.p12) (public, like the APK CI key).

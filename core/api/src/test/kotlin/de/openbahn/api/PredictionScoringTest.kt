...
class PredictionScoringTest {
    ...
    @Test
    fun probabilityOnTime_zeroToleranceMatchesExactBucket() {
        val distribution = listOf(0.1, 0.1, 0.8, 0.0)
        assertEquals(
            PredictionScoring.probabilityExactlyOnTime(distribution, offset = 2),
            PredictionScoring.probabilityOnTime(distribution, offset = 2, toleranceMinutes = 0),
            0.001,
        )
    }

    @Test
    fun probabilityOnTime_positiveToleranceSumsDelayBuckets() {
        val distribution = listOf(0.1, 0.1, 0.8, 0.0)
        assertEquals(1.0, PredictionScoring.probabilityOnTime(distribution, offset = 2, toleranceMinutes = 10), 0.001)
    }
